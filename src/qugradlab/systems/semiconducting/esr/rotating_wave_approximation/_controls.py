"""The control scheme for electron spin resonance (ESR) devices with a linear
array of qubits under the rotating wave approximation."""
...
class Controls(Device):
    """A class representing an ESR device with a linear array of
    qubits under the rotating wave approximation along with its control scheme.
    ...
    * :class:`qugradlab.physical_systems.semiconducting.esr.rotating_wave_approximation._systems.SpinChain`
    ...
    _rescale_rabi_drive: Callable[[np.ndarray[complex]], np.ndarray[complex]]
    """A function that rescales the Rabi drives.
    ...
    Returns
    -------
    NDArray[Shape[n_time_steps, n_drive_ctrl], complex]
        The rescaled control amplitudes
    """

    _rescale_J: Callable[[np.ndarray[complex]], np.ndarray[complex]]
    """A function that rescales the exchange couplings.
    ...
    Returns
    -------
    NDArray[Shape[n_time_steps, n_J_ctrl], complex]
        The rescaled control amplitudes
    """
    ...
    def __init__(self,
        ...
        """Initialises the control scheme for the ESR device with a linear array
        of qubits under the rotating wave approximation.
        ...
        """
        self._rescale_rabi_drive = \
            linear_rescaling.specify_parameters(min=-max_drive_strength,
                                                max=max_drive_strength)
        self.rescale_J = linear_rescaling.specify_parameters(min=J_min, max=J_max)
        super().__init__(zeeman_detunings, max_drive_strength, J_min, J_max)
    def _pre_processing(self,
        ...
        drive_frequencies : NDArray[Shape[n_drive_ctrl], complex]
            The detuning frequencies from the rotating frame frequency to
            modulate the control amplitude of the global Rabi-drive Hamiltonian
        J_ctrl_amp : NDArray[Shape[n_time_steps, n_J_ctrl], complex]
